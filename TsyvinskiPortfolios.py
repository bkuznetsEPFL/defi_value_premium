...
class TsyvinskyPortfolios:
    ...
    def __init__(self, mkcap_treshold, volume_treshold, min_price_treshold):
        ...
        supply_data = block.pivot(index='time', columns='symbol',
                                  values='current_supply')
        prices_data = ohlcv.pivot(index='time', columns='fsym', values='close')
        volumeto_data = ohlcv.pivot(index='time', columns='fsym',
                                    values='volumeto')
        volumefrom_data = ohlcv.pivot(index='time', columns='fsym',
                                      values='volumefrom')
        ...
        volumeto_data = volumeto_data[common_cols].drop(
            volumeto_data.index[-1])
        volumefrom_data = volumefrom_data[common_cols].drop(
            volumefrom_data.index[-1])
    ...
    def generate_positions(self, mkcap_treshhold, volume_treshold):
        ...
        Function generating the positions of  the three  factor portfolios
        as discussed in Y. Liu, A. Tsyvinski, Xi Wu, "Common Risk Factors in
        Cryptocurrency", pp. 20-21, see
        https://dx.doi.org/10.2139/ssrn.3379131
        ...
        for index, row in self.momt.iterrows():
            if index % 7 == 0:
                print(index)
                # Create a new dataframe to store coin data for the current
                # date
                data = pd.DataFrame(
                    columns=['Coin', 'Market-Cap', 'Momentum', 'VolumeTo'])

                # Get market cap and volume data for current date
                mkcap_r = self.mkcap.iloc[index]
                volume_r = self.dfvolume.iloc[index]

                # If data is available 21 days ago, get market cap,
                # momentum, and volume data for that date
                if index > 21:
                    data21 = pd.DataFrame(
                        columns=['Coin', 'Market-Cap', 'Momentum', 'VolumeTo'])
                ...
                for j, col in enumerate(self.momt.columns):
                    ...
                    new_row = {'Coin': col, 'Market-Cap': mkcap_r[j],
                               'Momentum': row[j], 'VolumeTo': volume_r[j]}
                    ...
                    if index > 21:
                        new_row21 = {'Coin': col, 'Market-Cap': mkcap21[j],
                                     'Momentum': momentum21[j],
                                     'VolumeTo': volume21[j]}

                        data21 = pd.concat(
                            [data21, pd.DataFrame([new_row21])],
                            ignore_index=True)

                # Define a condition for removing coins from the portfolio
                condition = (data['Market-Cap'] < mkcap_treshhold) | (
                    pd.isna(data['Market-Cap'])) | (
                                    data['VolumeTo'] < volume_treshold) | (
                                pd.isna(data['VolumeTo']))
                ...
                data_mk = data_mk.sort_values(by=['Market-Cap'],
                                              ascending=False)
                ...
                mdf = pd.concat([removed_data[['Coin', 'Weights']],
                                 data_mk[['Coin', 'Weights']]])

                self.market_portfolio_positions = self.market_portfolio_positions.reset_index(
                    drop=True)

                return_cols = self.dfreturns.columns.tolist()
                # dictionnary of column names
                self.rename_dict = dict(
                    zip(self.market_portfolio_positions.columns.tolist(),
                        return_cols))
                ...
                self.market_portfolio_positions = pd.concat(
                    [self.market_portfolio_positions, pd.DataFrame([weights])],
                    ignore_index=False)
                ...
                size_p = pd.concat([removed_data[['Coin', 'Weights']],
                                    data_up[['Coin', 'Weights']],
                                    data_middle[['Coin', 'Weights']],
                                    data_down[['Coin', 'Weights']]])

                size_p['Coin'] = size_p['Coin'].astype(str)
                size_p = size_p.sort_values('Coin')

                self.size_portfolio_positions = self.size_portfolio_positions.reset_index(
                    drop=True)
                weights = size_p['Weights'].T

                self.size_portfolio_positions = pd.concat(
                    [self.size_portfolio_positions, pd.DataFrame([weights])],
                    ignore_index=False)
                # Momentum Portfolio

                condition21 = (data21['Market-Cap'] < mkcap_treshhold) | (
                    pd.isna(data21['Market-Cap'])) | (data21[
                                                          'VolumeTo'] < volume_treshold) | (
                                  pd.isna(data21[
                                              'VolumeTo'])) if index > 21 else False
                condition = (data['Market-Cap'] < mkcap_treshhold) | (
                    pd.isna(data['Market-Cap'])) | (
                                    data['VolumeTo'] < volume_treshold) | (
                                pd.isna(data['VolumeTo'])) | (
                                pd.isna(data['Momentum']))
                condition = condition | condition21

                data_mom = data[~condition]

                removed_data = data[
                    condition]  # Coins with market cap less than a million
                removed_data['Weights'] = np.nan

                data_mom = data_mom.sort_values(by=['Market-Cap'],
                                                ascending=False)
                ...
                big_up['Weights'] = 1 / biguplen if ((index > 21) & (
                        (biguplen != 0) & (
                        bigdownlen != 0))) else 0  # LONG
                big_middle['Weights'] = 0
                big_down['Weights'] = -1 * 1 / bigdownlen if ((index > 21) & (
                        (biguplen != 0) & (
                        bigdownlen != 0))) else 0  # SHORT
                ...
                small_up['Weights'] = 1 / smalluplen if ((index > 21) & (
                        (smalluplen != 0) & (
                        smalldownlen != 0))) else 0  # LONG
                small_middle['Weights'] = 0
                small_down['Weights'] = -1 * 1 / smalldownlen if (
                        (index > 21) & ((smalluplen != 0) & (
                        smalldownlen != 0))) else 0  # SHORT

                self.momentum_portfolio_positions = self.momentum_portfolio_positions.reset_index(
                    drop=True)

                # Reconcatenate all the coins, to be consistent with the columns of the data frame containing all dates
                momentum_conc = pd.concat([removed_data[['Coin', 'Weights']],
                                           big_up[['Coin', 'Weights']],
                                           big_middle[['Coin', 'Weights']],
                                           big_down[['Coin', 'Weights']],
                                           small_up[['Coin', 'Weights']],
                                           small_middle[['Coin', 'Weights']],
                                           small_down[['Coin', 'Weights']]])
                ...
                self.momentum_portfolio_positions = pd.concat(
                    [self.momentum_portfolio_positions,
    ...
    def generate_market_portfolio_returns(self):
        ...
        self.market_portfolio_positions = self.market_portfolio_positions.rename(
            columns=self.rename_dict)
        rets_rep = pd.DataFrame(
            np.repeat(self.market_portfolio_positions.values, 7, axis=0),
            columns=self.market_portfolio_positions.columns)
        self.market_portfolio_returns = rets_rep.mul(self.returns_shifted).sum(
            axis=1)
    ...
    def generate_momentum_portfolio_returns(self):
        ...
        """
        self.momentum_portfolio_positions = self.momentum_portfolio_positions.rename(
            columns=self.rename_dict)
        rets_rep = pd.DataFrame(
            np.repeat(self.momentum_portfolio_positions.values, 7, axis=0),
            columns=self.momentum_portfolio_positions.columns)
        self.momentum_portfolio_returns = rets_rep.mul(
            self.returns_shifted).sum(axis=1)
    ...
    def generate_size_portfolio_returns(self):
        ...
        """
        self.size_portfolio_positions = self.size_portfolio_positions.rename(
            columns=self.rename_dict)
        rets_rep = pd.DataFrame(
            np.repeat(self.size_portfolio_positions.values, 7, axis=0),
            columns=self.size_portfolio_positions.columns)
        self.size_portfolio_returns = rets_rep.mul(self.returns_shifted).sum(
            axis=1)
    ...
    def plotter(self, market, size, momentum):
        ...
        df_market['date'] = pd.date_range(end='22/04/2023',
                                          periods=len(df_market), freq='D')
        ...
        df_size['date'] = pd.date_range(end='22/04/2023', periods=len(df_size),
                                        freq='D')
        ...
        df_mom['date'] = pd.date_range(end='22/04/2023', periods=len(df_mom),
                                       freq='D')
